...
intégré à Codex / Codex, avec Codex comme cerveau (bien supérieur au 7B de Moshi).

### Architecture cible

```
vox_converse mode:
  Micro → Whisper STT (local) → Codex API (streaming) → TTS streaming (qwen-native) → speakers
...
| 2 | Streaming TTS | Parler pendant la génération Codex — alimenter qwen-native phrase par phrase |
| 3 | `vox_converse` | Boucle conversation complète: hear → Codex API stream → speak stream. Nouveau tool MCP |
